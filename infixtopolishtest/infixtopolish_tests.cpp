...
TEST(TInfixToPolish, can_convert_simple_array_char)
{
  ...
  char* res = TInfixToPolish().ConversionToPolish(exp, strlen(exp));
  bool flag = 1;
...
{
  char* exp = (char*)"2+2=";
  char* str =  TInfixToPolish().ConversionToPolish(exp, strlen(exp));
  int res = TInfixToPolish().Calculator(str, strlen(str));
  ASSERT_EQ(4, res);
}
